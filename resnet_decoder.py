...
from layers import ConvBlock, upsample, Conv3x3, disp_to_depth
from gaussian_decoder import get_splits_and_inits, GaussianDecoder
...
class ResnetDecoder(nn.Module):
    ...
    def __init__(self, cfg, num_ch_enc, use_skips=True):
        ...
        self.num_ch_enc = num_ch_enc # np.array([64, 64, 128, 256, 512]) * 4
        self.num_ch_dec = np.array(cfg.model.backbone.num_ch_dec) # [32,32,64,128,256]

        self.split_dimensions, scales, biases = get_splits_and_inits(cfg) # guassian output layer parameters initialisation
        ...
        for i in range(4, -1, -1): # 4, 3, 2, 1, 0
            ...
            if self.use_skips and i > 0:
                num_ch_in += self.num_ch_enc[i - 1] # why -1?
            num_ch_out = self.num_ch_dec[i]
    ...
    def forward(self, input_features):
        ...
        for i in range(4, -1, -1):
            ...
            x = [upsample(x, mode=self.cfg.model.backbone.upsample_mode)] # mode=nearest [[[[0]]]] -> [[[[0,0],[0,0]]]]
            if self.use_skips and i > 0:
...
class ResnetDepthDecoder(nn.Module):
    ...
    def __init__(self, cfg, num_ch_enc, use_skips=True):
        ...
        self.cfg = cfg
        self.scales = cfg.model.scales # [0]
        self.use_skips = use_skips
        ...
        for s in self.scales: 
            out = Conv3x3(self.num_ch_dec[0], self.num_output_channels)
            self.convs[("outconv", s)] = out
            nn.init.xavier_uniform_(out.conv.weight, cfg.model.depth_scale)
            nn.init.constant_(out.conv_bias, cfg.model.depth_bias)
        ...
        elif cfg.model.depth_type == "depth_inc":  # current option
    ...
    def forward(self,input_features):
        ...
        for i in range(4, -1, -1):
            ...
            if i in self.scales:
                ...
                output = rearrange(self.activate(output), "b (n c) ... -> (b n) c ...", n=self.num_output_channels) # c = 1
